        ...
        <div class="px-4">
            <div class="bg-white overflow-hidden shadow-sm">
                <div class="p-4 bg-white border-b border-gray-200">
                    <h1 class="float-left h2">Inspecteur overzicht</h1>
                    @include('components.help-inspector-crud')
                    <a href="{{URL::to('/inspector/create')}}" class="float-right btn border" title="Toevoegen">
                        ...
                        Toevoegen</a>
                    <a href="{{URL::to('/inspector/archives')}}" class="float-right btn border mr-2" title="Archief">
                        Archief</a>
                    <p class="mb-5"></p>
                    <hr/>
                    {{--
                    ...
                    <div id="inspectors">
                        ...
                        @else
                            @foreach ($inspectors as $inspector)
                                <div id='inspector-field'
                                     class="row m-2 p-3 rounded border border-light shadow-sm bg-white">
                                    <div class="d-flex flex-column w-50">
                                        <div id="name"
                                             class="h5 m-0 fw-bold">
                                            Inspecteur {{ $inspector->first_name }} {{ $inspector->last_name }}</div>
                                        <p>{{ $inspector->phone_number }}</p>
                                    </div>
                                    <div class="d-flex flex-column justify-content-end w-50 text-right pb-2">
                                        <a id="{{$inspector->id}}"
                                           href="{{/*URL::to('/inspector/'.$inspector->id.'/details')*/ route('getInspectorEdit', $inspector->id) }}">
