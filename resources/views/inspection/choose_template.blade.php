                ...
                <div class="card-body">
                    ...
                    <div class="col-md-6">
                        @foreach($templates as $template)
                            <a href="{{URL::to('/inspection/'.$customer_id.'/'.$location->id.'/'.$template->id.'/create')}}" class="row w-75 h-10em vertical-center nounderline mb-3 rounded" style="background-color: {{$template->inspection_type()->color}}; border-color: {{$template->inspection_type()->color}}; color: #fff;"><h2 class="fw-bold"> <i class="fa fa-{{ $template->inspection_type()->icon()->name }}"></i>
